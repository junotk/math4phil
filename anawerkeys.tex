...
\section{位相}
\subparagraph{2.1}
下図のように，一点$a$から等しいユークリッド距離を持つ点は円（赤）に，等しいマンハッタン距離を持つ点は正方形（青）になる．
\[\begin{tikzpicture}
    ...
    \draw [red,thick](1.5,1.5)circle[radius=1];
    \draw [blue, thick, rotate around={45:(1.5,1.5)}] (0.79,0.79) rectangle (2.19,2.19);
\end{tikzpicture}\]

\subparagraph{2.2}
各世界間の距離は以下の通り．正定値性は対角がゼロであること，対称性は行列が対角を軸に対称な対称行列であることから明らか．三角不等式も，それぞれ計算すると成り立っている．
\begin{table}[htbp]
 \begin{tabular}[tb]{ccccc}
      & $w_1$ & $w_2$ & $w_3$ & $w_4$ \\ \hline
$w_1$ &  0  &  $\sqrt{3}$  &  $\sqrt{2}$  &  1  \\
$w_2$ &  $\sqrt{3}$  &  0  &  $\sqrt{3}$  &  $\sqrt{2}$  \\
$w_3$ &  $\sqrt{2}$  &  $\sqrt{3}$  &  0  &  $\sqrt{3}$  \\
$w_4$ &  1  &  $\sqrt{2}$  &  $\sqrt{3}$  &  0  
 \end{tabular}
...
\subparagraph{4.2}
$\{ \emptyset, \{w_1\}, \{w_2\}, \{w_1, w_2\}, \{w_1, w_3\}, \{w_1, w_4\},\{w_1, w_2, w_3\}, \{w_1, w_2, w_4\}, \{w_1, w_2, w_3\}$, \\
