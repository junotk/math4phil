...
\begin{rei}{概念と外延の間のガロア接続}{extension_galois}
事例\ref{rei:extension} で，概念の外延を与える関数$f$が，概念間の半順序$\langle C, \preceq \rangle$からモノの集合の冪集合の半順序$\langle \mathcal{P}(X), \subset \rangle$への単調写像を与えることを見た．
今この逆に，事物の集合から，その集合が属するような最も具体的な概念を返す関数$g$を考えよう．
例えば$g(\{ソクラテス, クレオパトラ, 西郷隆盛\})$は概念「人間」を，$g(\{ソクラテス, 忠犬ハチ公\})$は「哺乳類」を返す．
すると$g: \mathcal{P}(X) \to C$は$\langle \mathcal{P}(X), \subset \rangle$から$\langle C, \preceq \rangle$への
単調写像になる．

さらに，$f,g$は両順序の間のガロア接続である．
というのも，任意の概念$c \in C$について，その外延$f(c)$を含む最も具体的な概念は元の概念$c$なので，$g(f(c)) = c$がなりたち，よって$c \preceq g(f(c))$．
次に，任意の事物の集合$A \in  \mathcal{P}(X)$について，それらを含む最も具体的な概念の外延は，$A$以外にその概念に属する他の事物を足し合わせたものなので，$A \subset f(g(A))$が成り立ち，両者がガロア接続であることが示された．
