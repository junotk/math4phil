...
\begin{align*}
 x \leftarrow^{+} y 
 & \iff X \text{の要素からなる有限列} (x_0, \dots, x_n) \text{が存在し，} \\
 & (1) x = x_0 \text{ かつ } y = x_n, \\
 & (2) x_0 \leftarrow x_1, x_1 \leftarrow x_2, \dots, x_{n-1} \leftarrow x_n
\end{align*}
つまり，$x \leftarrow^{+} y$となるのは両者の間に，(1)両端が$x, y$であって，(2)その間の項が「$\leftarrow$」関係で結ばれている連鎖$x = x_0 \leftarrow x_1 \leftarrow \cdots \leftarrow x_n = y$が存在するときである．このように定義された新しい関係$\leftarrow^{+}$を，関係$\leftarrow$の\emph{推移閉包}(transitive closure)という．
推移閉包を取った$\leftarrow^{+}$は推移的なので，これは半順序となる．
\end{rei}  

\begin{renshu}{}{}
\begin{enumerate}
 \item $\leftarrow^{+}$は人格の同一性（の候補）にはまだ一歩たりない．というのも，同一性関係というからにはそれは同値関係（2章3節）であってほしいが，半順序は対称性を満たさないからだ．しかし，推移閉包を作ったのと同様，$\leftarrow^{+}$から対称閉包をとることによって，対称性を満たすような新たな関係（これを$\leftrightarrow^+$としよう）を作ることができる．それは以下のように定義できるだろう：
 \[ x \leftrightarrow^+ y \iff x, y \text{と} \leftarrow^{+} \text{を用いた条件} \]
 どのような条件を設定すればよいだろうか，考えてみよう．
...
\begin{renshu}{}{}
  下図のような二つの順序集合$X = \{x_1, x_2, x_3\}, Y = \{y_1, y_2\}$を考える．
  \[
  \begin{tikzpicture}
  \node (x1) at (1,0) {$x_1$};
  \node (x2) at (0,1.5) {$x_2$};
...
\end{tikzpicture}
\]
  このとき以下の関数$f: X \to Y$が単調写像であるか否かを答えよ．
  \begin{enumerate}
    \item $f(x_1) = y_1, f(x_2) = y_2, f(x_3) = y_2$
    \item $f(x_1) = y_1, f(x_2) = y_1, f(x_3) = y_2$
    \item $f(x_1) = y_1, f(x_2) = y_1, f(x_3) = y_1$
    \item $f(x_1) = y_2, f(x_2) = y_2, f(x_3) = y_1$
    \item $f(x_1) = y_2, f(x_2) = y_2, f(x_3) = y_2$
  \end{enumerate}
\end{renshu}  
単調写像でないのは4番だけである．というのもこのとき，$x_1 \preceq_X x_3$であるのに$f(x_1) = y_2 \not\preceq_Y y_1 = f(x_3)$だからだ．一方その他はみな単調写像である．
特に3番や5番はすべての点を1点（3は$y_1$, 5は$y_2$）につぶしてしまうが，順序は反射性$y_i \preceq y_i$を満たすので問題ない．
また2番では，$X$においては$x_2 \preceq_X x_3$ではないが，$Y$に飛ばした先では$f(x_2) = y_1 \preceq_Y f_2 = f(x_3)$と順序がついている．しかしこれも問題ない．単調写像の要件は，もし定義域$X$で二つが順序関係にあるならば，飛ばした先でそれらの間に順序関係がなければならない，ということだけである．定義域でもともと順序関係にないものが，値域で新しい順序関係を得てもその要件には反さないことに注意しよう．
...
最後に少し進んだ話題を一つ．
本章ではこれまで，二つの順序間の対応として単調写像と同型写像を見てきた．
単調写像$f: X \to Y$は，一方の順序$X$を他の順序$Y$へと対応づけるが，逆向きの対応（つまり$Y$が$X$にどう対応するか）については何も言わない．
これが同型写像になるとき，逆向きの対応$g: Y \to X$が存在するが，このとき$X$と$Y$は同型，つまり順序としては全く同じになってしまう．
同型よりもうちょっと緩い双方向の対応関係，つまり$X, Y$は順序として同一ではないものの，単調写像によってお互いに対応付けられている，というような状況はないのだろうか．
以下に紹介するガロア接続は，そのような状況を捉えるものである．

\begin{dfn}{ガロア接続}{}
$\langle X, \preceq_X \rangle, \langle Y, \preceq_Y \rangle$を順序とする．
単調写像（減少であってもよい）の組$f:X \to Y, g:Y \to X$が存在し，任意の$x \in X, y \in Y$に対し，
\begin{equation}
   x \preceq_X g(y)  \ \ \iff \ \ f(x) \preceq_Y y \tag{G1} 
   \label{eqn:galois1}
\end{equation}
が満たされているとき，$f$と$g$を$X$と$Y$の間の\emph{ガロア接続}(Galois connection)と呼ぶ．
\end{dfn}

つまり任意の$x$について，それが$g$で飛ばされてきた$g(y)$よりも小さいのは，$x$を$f$で$Y$に飛ばした$f(x)$が，$y$より小さくなるとき，その時に限る．これは言葉で考えるより，図\ref{fig:galois}（左）で見たほうが理解しやすいかもしれない．
...
\begin{figure}[h]
  ...
  \[\begin{tikzpicture}
      ...
      \draw (x) -- (gy) node[midway, above, rotate=90]{$\preceq_X$};
      \draw (fx) -- (y) node[midway, below, rotate=90]{$\preceq_Y$};
      %s2
      \draw[->, bend right=15] (x) to node[midway, below]{$f$}(fx);
      \draw[->, bend right=15] (y) to node[midway, above]{$g$} (gy);
  ...
  \[\begin{tikzpicture}
      \node (x) at (0, 3) {$x$};
      \node (fx) at (2.5,3.5) {$f(x)$};
      \node (gfx) at (0,4) {$g(f(x))$};
      \node (y) at (2.5, 1.5) {$y$};
      \node (gy) at (0,1) {$g(y)$};
      \node (fgy) at (2.5,0.5) {$f(g(y))$};
  
      \draw (x) -- (gfx);
      \draw (y) -- (fgy);
      %s2
      \draw[->, bend right=15] (x) to node[midway, below]{$f$}(fx);
      \draw[->, bend right=15] (fx) to node[midway, above]{$g$} (gfx);
      \draw[->, bend right=15] (y) to node[midway, above]{$g$} (gy);
      \draw[->, bend right=15] (gy) to node[midway, below]{$f$} (fgy);

      \draw[dashed] (x) -- ++(0,-0.5);
      \draw[dashed] (gfx) -- ++(0,0.5);
      \draw[dashed] (fx) -- ++(0,-1);
      \draw[dashed] (fx) -- ++(0,1);

      \draw[dashed] (y) -- ++(0,0.5);
      \draw[dashed] (fgy) -- ++(0,-0.5);
      \draw[dashed] (gy) -- ++(0,-1);
      \draw[dashed] (gy) -- ++(0,1);
  \end{tikzpicture}\]
  \hfill
  ...
  \begin{minipage}[t]{0.32\textwidth}
      ...
      \draw[dashed] (fx) -- ++(0,1);
      \draw[->, bend right=15] (x) to node[midway, below]{$f$} (fx);
      \draw[->, bend right=15] (fx) to node[midway, above]{$g$} (x);
      ...
      \draw[dashed] (gy) -- ++(0,1);
      \draw[->, bend right=15] (y) to node[midway, above]{$g$} (gy);
      \draw[->, bend right=15] (gy) to node[midway, below]{$f$} (y);

  \end{tikzpicture}\]
  \hfill
  \end{minipage}
  \label{fig:galois}
  \caption{左と中央：ガロア接続の二つの等価な定義．右：順序同型．各コラムとも，左側が順序$X$，右側が順序$Y$を示す．}
\end{figure}

ところでガロア接続は，次のように定義することもできる：
\begin{dfn}{ガロア接続のもう一つの定義}{}
単調写像の組$f:X \to Y, g:Y \to X$が，任意の$x \in X, y \in Y$に対し，
\begin{equation}
  x \preceq_X g(f(x)), \ \ \ \ \ \ f(g(y)) \preceq_Y y  \tag{G2}
  \label{eqn:galois2}
\end{equation}
を満たす．
\end{dfn}

$f:X \to Y$と$g:Y \to X$を合成した$g \circ f$は，$X$から$Y$を経由して$X$に戻ってくる写像である．
上の定義\ref{eqn:galois2}は，任意の$x$は，このように$g \circ f$で行って戻ってくると出発点以上になっている（同じでも良い），ということを言っている．
また逆に，任意の$y$は，$f \circ g$で行って戻すと出発点以下になる（同じく，同じでも良い）．
これはちょうど，図\ref{fig:galois}（中央）のような状況である．
一方，図\ref{fig:galois}（右）は，順序同型を表している．順序同型では，行って戻ったら出発点にぴったり一致しなければならないが，ガロア接続は$X$側ではそれが「上ブレ」することを許し，$Y$側では「下ブレ」することを許す，という意味でより緩い対応関係になっている．

% 定義の同値性
式(\ref{eqn:galois1})と(\ref{eqn:galois2})は同値であるので，これらの定義は実質的に同じである．
まず(\ref{eqn:galois1})$\Rightarrow$(\ref{eqn:galois2})を示そう．
任意の$x \in X$について，$y := f(x)$と置いてみる．
すると$\preceq_Y$の反射性より，$f(x) \preceq_Y y$である．
(\ref{eqn:galois1})の条件より，これは$x \preceq g(y)$と同値，すなわち$x \preceq g(f(x))$である．
$f(g(y)) \preceq_Y y$についても同様に示せる．

次に(\ref{eqn:galois2})$\Rightarrow$(\ref{eqn:galois1})を示す．
そのために(\ref{eqn:galois2})を仮定して，まず$x \preceq_X g(y) \Rightarrow f(x) \preceq_Y y$を示す．
そのため$x \preceq_X g(y)$と仮定すると，$f$は単調写像なので$f(x) \preceq_Y f(g(y))$．
(\ref{eqn:galois2})より$f(g(y)) \preceq_Y y$なので，$f(x) \preceq_Y y$が従う．
逆向き$f(x) \preceq_Y y \Rightarrow x \preceq_X g(y)$も全く同様である．

ガロア接続のような構造は，数学では非常に多く出てくる（実際これは，圏論でいう「随伴」の顕著な一事例となっている）．
しかしそれだけでなく，ガロア接続は哲学的にも非常に大きな含意と，広い応用の可能性を持っている．
というのはそれは，ある一つの体系と，他の体系を，自然な仕方で接合してくれるからである．

\begin{rei}{外延と内包の双対性再訪}{duality_again}
  事例\ref{rei:duality}では，モノの集合$A \subset X$から，それらのモノが共有する性質の集合への写像
  \begin{align*}
    %f^\ast &: \mcalP(X) \to \mcalP(Y) \\
  ...
  \[
    A \subset g^\ast(f^\ast(A)) \ \ \ \ \ \ f^\ast(g^\ast(P))　\subset P 　
  \]
  がなりたつ．
